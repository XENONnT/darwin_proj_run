...
def get_likelihood_config(exposure=1.,signal_config={},**kwargs):
    source_configs = [
            dict(
                name="er",
                label="ER events from Pb214Spectrum.json",
                templatename = get_resource_filename("NEST_dummyresponse.hdf5"),
                histname="er",
                spectrum=get_resource_filename("spectra/Pb214Spectrum.json"),
                histogram_multiplier = 3.5,
...
class InferenceObject:
    def __init__(self, wimp_mass = 50,
        ...
        if interaction=="peak":
            signal_spectrum = sps.norm(wimp_mass,0.01*wimp_mass).pdf
        signal_config = dict(
            histname= 'nr',
            label= 'WIMP events',
            name= 'signal',
        ...
        for parameter in ["er","nrnusun"]:
            if parameter in parameter_uncerts.keys():
